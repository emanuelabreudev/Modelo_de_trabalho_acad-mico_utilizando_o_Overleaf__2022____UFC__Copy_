...
Mais recentemente, \citeonline{marquesfiori2023} consolidaram uma evolução do método a partir das dificuldades observadas em sua aplicação. Essa evolução envolveu a revisão das cartas, com a remoção de conteúdos redundantes e a combinação de cartas de conteúdo semelhante, além da proposta de um \textit{checklist} e de um \textit{framework} para orientar a seleção das cartas conforme as características dos usuários e do sistema. É essa versão evoluída, baseada em cartas e em uma etapa de filtragem por \textit{checklist}, que fundamenta os estudos de aplicação analisados neste trabalho.
